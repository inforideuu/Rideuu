...
export const InteractiveMap: React.FC<{
  ...
  const markerRiderRef = useRef<any>(null);
  const markerPickupRef = useRef<any>(null);
  const markerDropoffRef = useRef<any>(null);
  const routeLineRef = useRef<any>(null);
  const routeBorderRef = useRef<any>(null);
  const heatCirclesRef = useRef<any[]>([]);
  const surgeMarkersRef = useRef<any[]>([]);
  const floodZonesRef = useRef<any[]>([]);
  ...
  // Fetch routing path geometry from OSRM depending on current active ride stage
  useEffect(() => {
    ...
    if (activeRide.stage !== "pickup") {
      // In transit or OTP stage: route goes from Pickup to Dropoff
  ...
  // Render & Update Leaflet map elements
  useEffect(() => {
    if (!leafletLoaded || !mapContainerRef.current) return;
    const L = (window as any).L;
    if (!L) return;

    // Driver base location coordinates
    ...
    if (!leafletMapRef.current) {
      ...
      // Light street map theme matching Google Maps
    ...
    // Clear existing overlay markers and lines
    ...
    // 2. Draw Flood warning indicators
    if (floodAlert) {
    ...
    // Calculate moving driver position coordinates
    ...
    if (activeRide.request) {
      ...
      if (activeRide.stage === "pickup") {
        ...
        progress = 0.4; // 40% along the path to pickup
      } else if (activeRide.stage === "otp") {
        startLat = pickupCoords[0];
        startLon = pickupCoords[1];
        endLat = dropoffCoords[0];
        endLon = dropoffCoords[1];
        progress = 0.0; // stationary at pickup
      } else {
        startLat = pickupCoords[0];
        startLon = pickupCoords[1];
        endLat = dropoffCoords[0];
        endLon = dropoffCoords[1];
        progress = Math.min(0.1 + (activeRide.currentStepIndex * 0.3), 1.0);
      }
    }
    ...
    // 3. Draw active ride route line
    if (activeRide.request && routeCoords.length > 0) {
      // Dark blue border line
      ...
      // Render Dropoff red location pin (only if stage is trip or otp)
      if (activeRide.stage !== "pickup") {
  ...
  return (
    <div className={`relative w-full ${height} bg-slate-100 rounded-3xl overflow-hidden shadow-inner border border-slate-200`}>
      <div ref={mapContainerRef} className="absolute inset-0 w-full h-full z-10" />

      {/* Floating HUD elements over Leaflet map */}
      <div className="absolute bottom-3 left-3 right-3 bg-slate-900/90 border border-white/10 rounded-2xl p-2.5 backdrop-blur flex justify-between items-center text-white z-20 pointer-events-auto shadow-md">
        <div className="flex items-center gap-2">
          <div className={`h-2.5 w-2.5 rounded-full ${rainActive ? "bg-blue-400" : "bg-green-500 animate-pulse"}`} />
          <span className="text-[10px] font-bold tracking-wide uppercase">
            {rainActive ? (language === "ta" ? "மழை நேரடி சவாரி" : "RAIN INCENTIVES ACTIVE") : (language === "ta" ? "நேரடி வரைபடம்" : "LIVE STREET MAP")}
          </span>
        </div>
        <div className="flex gap-2">
          {floodAlert && (
            <div className="flex items-center gap-1 bg-blue-500/20 text-blue-400 border border-blue-500/30 px-2 py-0.5 rounded-lg text-[9px] font-bold">
              💧 {language === "ta" ? "வெள்ளம்" : "FLOOD"}
            </div>
          )}
          {activeRide.request && (
            <div className="bg-primary/20 text-primary border border-primary/30 px-2 py-0.5 rounded-lg text-[9px] font-bold">
              {language === "ta" ? "பயணத்தில்" : "ON SHIFT"}
            </div>
          )}
        </div>
      </div>
